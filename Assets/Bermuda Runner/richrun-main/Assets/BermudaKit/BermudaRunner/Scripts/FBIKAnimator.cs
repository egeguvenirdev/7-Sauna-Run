...
public class FBIKAnimator : MonoBehaviour
{
    ...
    [SerializeField] private HandPoser _rightHandPoser;
    [SerializeField] private HandPoser _leftHandPoser;
    ...
    private void Awake()
    {
        if(_lookAtIK)
        {
            ...
            _aimTarget.position = _lookAtIK.solver.head.transform.position + _lookAtIK.solver.head.forward * 3f;
            _aimTarget.SetParent(_fbik.transform, true);
        }
        ...
        if(_lookAtIK)
        {
            _lookAtIK.solver.target = _aimTarget;
        }
    ...
    public void MoveAimTo(Vector3 position, float duration, bool relative)
    {
        //_leftHandTarget.DOKill();
        _aimTarget.DOKill();
        _aimTarget.DOMove(position, duration).SetRelative(relative);
    }
    ...
    public void SqueezeHand(bool rightHand, float duration)
    {
        if (rightHand)
        {
            DOTween.To(() => _rightHandPoser.weight, x => _rightHandPoser.weight = x, 1f, duration);
        }
    }

    public void LeaveHand(bool rightHand, float duration)
    {
        if (rightHand)
        {
            DOTween.To(() => _rightHandPoser.weight, x => _rightHandPoser.weight = x, 0f, duration);
        }
    }

    public void SetAimIKWeight(float weight, float duration)
    {
        if(_lookAtIK)
        {
            DOTween.To(() => _lookAtIK.solver.IKPositionWeight, x => _lookAtIK.solver.IKPositionWeight = x, weight, duration);
        }
    }
